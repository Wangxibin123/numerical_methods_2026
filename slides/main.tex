\documentclass[aspectratio=169,11pt]{beamer}

\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{amsmath,amssymb}
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{ctex}   % comment out if ctex unavailable; English alt provided below in [..]

\usetheme{Madrid}
\usecolortheme{seahorse}
\setbeamertemplate{navigation symbols}{}
\setbeamertemplate{footline}[frame number]
\setbeamertemplate{caption}[numbered]

\title[NYC TLC Taxi Rebalancing]{城市出租车供需错配与空驶再平衡优化}
\subtitle{基于 NYC TLC Yellow Taxi 分区小时流数据的预测—优化模型}
\author{第一组 \\ \small 23 信科 陈龙 \quad 22 光华 王溪斌 \quad 23 元培 汤平之}
\institute{数值分析期末项目（2026 春）}
\date{\today}

\begin{document}

% ----------------------------------------------------------
\begin{frame}
\titlepage
\end{frame}

% 1
\begin{frame}{现实问题}
城市出租车系统在时间和空间上存在 \alert{显著供需错配}：
\begin{itemize}
\item 工作日晚高峰 Manhattan 核心区 “车少人多”。
\item 夜间机场区域 / Manhattan 间存在固定流入流出失衡。
\end{itemize}
\vspace{0.5em}
\textbf{研究问题}：能否仅用公开数据，先预测下一小时各区域需求，
再求解一个空车再平衡的 LP 问题，使未满足需求降低、空驶成本可控？
\end{frame}

% 2
\begin{frame}{数据来源}
\begin{itemize}
\item NYC Taxi \& Limousine Commission Yellow Taxi Trip Records (Parquet)
\item 默认范围：2024-01 \textendash{} 2024-03（可扩到 2024-06）
\item 时间粒度：1 hour；空间粒度：top-50 taxi zone（按 \emph{训练期} pickup 量选）
\item Taxi Zone Lookup CSV + 可选 Shapefile
\end{itemize}
\end{frame}

% 3
\begin{frame}{Zone--Hour 面板}
\[
P_{i,t}=\text{pickup count},\quad
Q_{i,t}=\text{dropoff count},\quad
B_{i,t}=Q_{i,t}-P_{i,t}.
\]
特征：lag\_1h/2h, rolling 6h/24h, same-hour-prev-day/week, $\sin/\cos$ 时编码, weekday, is\_weekend。

\vspace{0.5em}
Train/Val/Test 按时间 70/15/15 切分 — \textbf{不做随机切分}（避免泄漏）。
\end{frame}

% 4
\begin{frame}{需求预测：两个模型}
\textbf{Model 1 — 历史均值 baseline}
\[
\widehat P_{i,t+1}=\overline{P}\big|_{\text{zone}=i,\;\text{wd}=\text{wd}(t+1),\;\text{hod}=\text{hod}(t+1)}.
\]

\textbf{Model 2 — Ridge regression}
\[
\widehat\beta=(X^\top X+\alpha I)^{-1}X^\top y,\quad
\widehat P_{i,t+1}=\max(z_{i,t}^\top\widehat\beta,0).
\]

指标：MAE, RMSE, sMAPE（在测试期）。
\end{frame}

% 5
\begin{frame}{供需缺口的定义}
\[
D_{j,t+1}=\max(\widehat P_{j,t+1}-Q_{j,t},0),
\quad
S_{i,t}=\max(Q_{i,t}-\widehat P_{i,t+1},0).
\]

\textbf{关键诚实}：$Q_{i,t}$ 是 \emph{供给代理}，不是平台真实空车库存。

\vspace{0.5em}
成本矩阵 $c_{ij}$ = 训练期 OD 行程 trip\_distance 中位数；
缺失 → 同 borough 中位数 → 全局 $\times 1.5$。
\end{frame}

% 6
\begin{frame}{LP 再平衡模型}
\[
\boxed{\;\min_{x,u}\;\;\sum_{i,j} c_{ij}\, x_{ij}+\lambda\sum_j u_j\;}
\]
\[
\text{s.t.}\quad
\sum_j x_{ij}\le S_i\;\forall i,\quad
\sum_i x_{ij}+u_j\ge D_j\;\forall j,\quad
x,u\ge 0.
\]
连续 relaxation。$x_{ij}$ 解释为期望调度强度。
$\lambda$ 控制 “未满足需求” 的惩罚权重，单位 = mile / unit。
\end{frame}

% 7b — NEW: lambda 的经济学依据
\begin{frame}{$\lambda$ 的经济学依据 \& 敏感性}
\small
\textbf{$\lambda$ 含义}：单位为英里，
等于 “漏掉 1 个 trip” 相当于多少英里空驶。
\vspace{0.4em}

\textbf{由 NYC TLC 2024 Yellow Taxi 标准费率推导}：
\begin{center}\small
\begin{tabular}{lcc}
\toprule
视角 & 推导 & $\lambda$ (mi) \\
\midrule
司机 & 净收入 \$12 / 空驶单位成本 \$0.40/mi & $\approx 30$ \\
社会 & (\$12 + 客户机会成本 \$5) / \$0.40 & $\approx 42$ \\
平台 & 折扣率回报法 & $20$–$25$ \\
\bottomrule
\end{tabular}
\end{center}

\textbf{可辩护区间} $\lambda \in [20, 42]$，
我们以 $\lambda = 20$（保守下界）为默认值；
20 个 $\lambda$ 值的全测试小时 sweep 显示
\emph{整个 [20, 200] 区间已饱和到 unmet=77.7 / srv=0.949}——
对参数选择鲁棒。
\end{frame}

% 8
\begin{frame}{Baselines}
\begin{itemize}
\item \textbf{Baseline 0}: 不调度，$X = 0$。
\item \textbf{Baseline 1}: 贪心最近邻
    \begin{itemize}\small
    \item 按 $D$ 从大到小遍历缺口 zone
    \item 每次从最便宜的剩余 supply zone 调过来，直到该 zone 缺口填满或 supply 用完
    \end{itemize}
\item \textbf{Main}: LP \verb|linprog| 全局最优。
\end{itemize}
\end{frame}

% 10
\begin{frame}{北太天元实现流程}
\small
\texttt{run\_all.m} 一键串联，全部在 Baltamatica 2025 社区版上跑通：
\begin{enumerate}\itemsep=2pt
\item 读取 7 个 processed CSV（\texttt{load\_processed\_data.m}）
\item 验证 70/15/15 时间切分
\item 拟合 historical mean + Ridge 两个预测器
\item 对每个测试小时构造 $(S, D, Q, P^{\text{true}})$，
      同时跑 no-rebalance / greedy / LP 三种方案
\item $\lambda$ 敏感性扫描（20 个 $\lambda$ $\times$ 24 小时 $=$ 480 个 LP）
\item Monte Carlo 200 次 Poisson 扰动稳健性
\item 导出 5 张图源 CSV $\rightarrow$ Python matplotlib 渲染
\end{enumerate}
\end{frame}

% 9
\begin{frame}{实验结果（之一）— 调度对比}
\begin{figure}
\includegraphics[width=0.82\textwidth]{../results/figures/fig_dispatch_comparison.png}
\end{figure}
\vspace{-0.5em}
\small
LP 与 greedy 在 unmet 上几乎重合，但 LP 空驶成本平均低 \textbf{2 – 5 倍}：
hour 14: $5033 \to 1540$ mi（\textbf{3.3$\times$}），
hour 18: $3197 \to 707$ mi（\textbf{4.5$\times$}），
hour 17: $921 \to 429$ mi（\textbf{2.1$\times$}）。
\end{frame}

% 10
\begin{frame}{实验结果（之二）— Top LP flows}
\begin{figure}
\includegraphics[width=0.78\textwidth]{../results/figures/fig_top_flows.png}
\caption{Top-15 LP 调度流量累计于 24 个测试小时。
前 5 条几乎全部是 Manhattan 住宅区 $\to$ 机场，
6 - 15 条是 Manhattan 内部住宅区 $\to$ 商务区。}
\end{figure}
\end{frame}

% 11
\begin{frame}{Monte Carlo 稳健性}
\begin{figure}
\includegraphics[width=0.85\textwidth]{../results/figures/fig_monte_carlo_boxplot.png}
\caption{固定调度方案 $X^*$ 在 200 个 Poisson 扰动场景下的指标分布。}
\end{figure}
\end{frame}

% 12
\begin{frame}{结论与局限}
\textbf{结论}：LP 与 greedy 达到 \emph{相同} 真实 unmet 水平，
但 LP 空驶成本平均低 \textbf{2 – 5 倍}；
Monte Carlo 200 次 Poisson 扰动下结论保持稳定。

\vspace{0.5em}
\textbf{六项局限}（均在报告中具体讨论）：
\begin{itemize}\small\itemsep=1pt
\item dropoff 流是供给代理 — 不是平台实时空车库存。
\item 没有天气 / 道路拥堵 / 大型活动等外生数据。
\item LP 连续 relaxation，$x_{ij}$ 是期望调度强度（部署需取整）。
\item 成本矩阵基于历史 trip\_distance 中位数，未含实时路况。
\item 评估假设调度瞬时执行，未模拟驾驶时延。
\item Ridge 不带 zone embedding，性能弱于强基线历史均值。
\end{itemize}

\vspace{0.5em}
完整代码与复现命令：\verb|github.com/Wangxibin123/numerical_methods_2026|
\end{frame}

\begin{frame}
\begin{center}
{\Huge 谢谢}\\
\vspace{1em}
{\large 问题与讨论}\\
\vspace{2em}
{\small 第一组｜23 信科 陈龙 · 22 光华 王溪斌 · 23 元培 汤平之}
\end{center}
\end{frame}

\end{document}
